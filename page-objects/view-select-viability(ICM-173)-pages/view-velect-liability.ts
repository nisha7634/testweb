...
export class viewSelectLiabilityPage {
  ...
  constructor(page: Page, context: BrowserContext) {
    ...
    this.toggleBtn = page.locator('//input[@id="emergencyToggle-input"]')
  ...
  async enterFOB() {
    // Wait for the FOB dropdown to be visible
    await expect(this.page.getByLabel('FOB').locator('span')).toBeVisible();

    // Click the FOB dropdown to display the options
    await this.page.getByLabel('FOB').locator('span').click();

    // Wait for the 'Liability' option to be visible
    await expect(this.page.locator('mat-option').filter({ hasText: 'Liability' })).toBeVisible();
    await expect(this.page.locator('mat-option').filter({ hasText: 'Travel Insurance' })).toBeVisible();

    // Click the 'Liability' option
    await this.page.locator('mat-option').filter({ hasText: 'Liability' }).click();
  }


  async verifyOtherValueinFOC() {
    // Click the FOC dropdown
    await this.page.getByLabel('FOC').locator('div').nth(3).click();

    // Wait for the 'Other' option to be visible
    await expect(this.page.getByText('Other')).toBeVisible();

    // Click the 'Other' option
    await this.page.getByText('Other').click();

    // Verify that 'FOC' now contains the text 'Other'
  ...
  async enterProviderValue() {
    // Use the unique id attribute to locate the Provider input field
    const providerInput = this.page.locator('#providerAC12');

    // Wait for the specific input to be visible
    await expect(providerInput).toBeVisible({ timeout: 20000 });

    // Click the specific input to focus it
    await providerInput.click();

    // Fill in the value "Lawyer"
    await providerInput.fill('Lawyer');

    // Optionally, wait for any necessary loading or processing
    await this.page.waitForTimeout(1000);  // Adjust timeout as needed for loading to complete

    // Verify that the input field contains "Lawyer"
    const value = await providerInput.inputValue();
    expect(value).toContain('Lawyer');
  }
  ...
  async selectSpecificAssessment_generalSymptoms() {
    //await this.page.getByLabel('Specific Assessment*').click();
    await this.page.getByLabel('Specific Assessment*').fill('R68.89 Other general symptoms and sign ');
    await this.page.getByLabel('Specific Assessment*').press('Space')
  }
